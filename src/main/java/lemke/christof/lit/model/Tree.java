...
public record Tree (List<Entry> entries) implements DbObject {
    ...
    @Override
    public byte[] data() {
        List<byte[]> chunks =  entries.stream().map(e -> {
            byte[] mode = e.mode().getBytes(StandardCharsets.UTF_8);
            byte[] oid = HexFormat.of().parseHex(e.oid());
            byte[] name = e.name().toString().getBytes(StandardCharsets.UTF_8);
        ...
        int length = chunks.stream().map(b -> b.length).reduce((l1, l2) -> l1 + l2).get();
